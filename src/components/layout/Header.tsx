    ...
    <header className="bg-[#0b1e13] border-b border-[#1c472e] sticky top-0 z-40 h-12 select-none shadow-sm">
      <div className="max-w-[1220px] mx-auto h-full px-2 sm:px-4 flex items-center justify-between">
        ...
        <div className="flex items-center space-x-3 sm:space-x-4 text-xs">
          ...
          {currentUser ? (
            <div className="flex items-center space-x-2 bg-[#06150c] pl-2.5 pr-1.5 py-1 rounded-full border border-[#1f4f34]">
              <div className="flex items-center space-x-1.5">
                ...
                {userRole === 'editor' && (
                  <span className="bg-amber-500/20 text-amber-300 border border-amber-500/40 text-[9px] font-black uppercase px-1.5 py-0.2 rounded-full">
                    Editor
                  </span>
                )}
              </div>
              <button
                type="button"
                onClick={logout}
                className="ml-1 p-1 hover:bg-[#153e26] text-gray-400 hover:text-red-400 rounded-full transition-colors flex items-center space-x-1"
                title="Cerrar sesión"
              >
            ...
            <button
              ...
              className="px-3 py-1 bg-[#153e26] hover:bg-[#1e5836] text-[#22c55e] hover:text-white border border-[#22c55e]/50 rounded-full text-xs font-bold transition-all flex items-center space-x-1.5 shadow-xs"
          ...
          {userRole === 'editor' ? (
            <div className="flex items-center space-x-1 bg-[#06150c] border border-amber-500/40 px-2 py-0.5 rounded-full shadow-xs">
              <span className="text-[10px] text-amber-300 font-bold uppercase tracking-wider pl-1">Año:</span>
              <input
                type="text"
                value={globalYear}
                onChange={(e) => setGlobalYear(e.target.value)}
                className="w-14 bg-[#0a1e13] text-amber-300 font-mono font-black text-xs text-center border border-[#1f4f34] rounded-md px-1 py-0.5 focus:border-[#22c55e] focus:outline-none"
                title="Editor general del año de la página (ej. 1974). Al cambiarlo se sincroniza en toda la web."
                placeholder="1974"
              />
            </div>
          ) : (
            <div className="flex items-center space-x-1 bg-[#06150c] border border-[#1f4f34] px-2.5 py-1 rounded-full text-xs font-mono font-bold text-amber-300 shadow-xs">
              <span className="text-gray-400 font-sans text-[10px] font-bold">AÑO</span>
              <span>{globalYear}</span>
            </div>
          )}

          <span className="text-[#1c472e]">|</span>
          ...
          {/* Opciones exclusivas del EDITOR */}
          {userRole === 'editor' && (
            <>
              <span className="text-[#1c472e] hidden sm:inline-block">|</span>
              <button
                onClick={() => setShowAdminModal(true)}
                className="text-gray-300 hover:text-[#22c55e] font-semibold transition-colors flex items-center space-x-1"
